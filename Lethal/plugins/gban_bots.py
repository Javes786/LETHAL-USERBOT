...
@Lethal.on(admin_cmd(pattern="botgban ?(.*)"))
@Lethal.on(sudo_cmd(pattern="botgban ?(.*)", allow_sudo=True))
async def _(event):
...
@Lethal.on(admin_cmd(pattern="botungban ?(.*)"))
@Lethal.on(sudo_cmd(pattern="botungban ?(.*)", allow_sudo=True))
async def _(event):
    if Config.G_BAN_LOGGER_GROUP is None:
        await eor(
            event,
