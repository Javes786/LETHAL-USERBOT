...
from Lethal import ALIVE_NAME, CMD_HELP
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        msg = None
        if reason is not None and lethal == "True":
        ...
        if Var.PRIVATE_GROUP_ID:
            ...
            if not event.is_private:
                ...
                try:
                    ...
                    await Lethal.send_message(
                        Var.PRIVATE_GROUP_ID,
                        f"Please add {MYBOT} here for afk tags to work.",
                    )


@Lethal.on(admin_cmd(pattern=r"afk ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK
    ...
    if not USER_AFK:
        if event.reply_to_msg_id:
            ...
            media = await Lethal.download_media(reply_message, "AFK_media")
            try:
...
@Lethal.on(events.NewMessage(outgoing=True))
