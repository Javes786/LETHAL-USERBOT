...
@Lethal.on(admin_cmd(outgoing=True, pattern="kang"))
@Lethal.on(sudo_cmd(pattern="kang", allow_sudo=True))
async def kang(args):
            ...
            async with bot.conversation("Stickers") as conv:
                ...
                while "120" in x.text:
                    ...
                    packnick = (
                        f"{telename} Vol.{pack}"
                        if telename
                        else f"@{user.username}'s Lethal Vol.{pack}"
                    )
                    await args.edit(
                        "`Switching to Pack "
                        + str(pack)
...
@Lethal.on(admin_cmd(outgoing=True, pattern="stkrinfo"))
@Lethal.on(sudo_cmd(pattern="stkrinfo", allow_sudo=True))
