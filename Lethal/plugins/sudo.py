...
@Lethal.on(admin_cmd(pattern="sudo"))
async def sudo(event):
    sudo = "True" if Config.SUDO_USERS else "False"
    users = os.environ.get("SUDO_USERS", None)
    if sudo == "True":
        await eor(event, f"**Lethal**\nSudo - `Enabled`\nSudo user(s) - `{users}`")
    else:
        await eor(event, f"**Lethal**\nSudo - `Disabled`")


@Lethal.on(admin_cmd(pattern="prefix"))
...
@Lethal.on(admin_cmd(pattern="addsudo(?: |$)"))
async def tb(event):
