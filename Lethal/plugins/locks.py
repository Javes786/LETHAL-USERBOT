...
from Lethal.plugins.sql_helper.locks_sql import get_locks, is_locked, update_lock
from Lethal.utils import admin_cmd


@Lethal.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@Lethal.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@Lethal.on(admin_cmd(pattern="unlock ?(.*)"))
@Lethal.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Lethal.on(admin_cmd(pattern="currenabledlocks"))
@Lethal.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@Lethal.on(events.MessageEdited())  # pylint:disable=E0602
@Lethal.on(events.NewMessage())  # pylint:disable=E0602
...
@Lethal.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
