...
@Lethal.on(admin_cmd(outgoing=True, pattern=r"gs (.*)"))
@Lethal.on(sudo_cmd(allow_sudo=True, pattern=r"gs (.*)"))
async def gsearch(q_event):
    """For .google command, do a Google search from @LethalHelp."""
...
@Lethal.on(admin_cmd("duckduckgo (.*)"))
@Lethal.on(sudo_cmd("duckduckgo (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Lethal.on(admin_cmd(pattern="ggl (.*)"))
@Lethal.on(sudo_cmd(pattern="ggl (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
