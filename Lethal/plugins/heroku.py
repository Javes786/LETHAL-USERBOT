...
from Lethal import CMD_HELP, CMD_HNDLR

Heroku = heroku3.from_key(Var.HEROKU_API_KEY)
heroku_api = "https://api.heroku.com"


@Lethal.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
@Lethal.on(admin_cmd(pattern="usage"))
@Lethal.on(sudo_cmd(pattern="usage", allow_sudo=True))
...
@Lethal.on(admin_cmd(pattern="info heroku"))
@Lethal.on(sudo_cmd(pattern="info heroku", allow_sudo=True))
...
@Lethal.on(admin_cmd(outgoing=True, pattern=r"logs"))
@Lethal.on(sudo_cmd(allow_sudo=True, pattern=r"logs"))
async def _(givelogs):
    ...
    with open("logs-Lethal.txt", "w") as log:
