...
@Lethal.on(admin_cmd(pattern="song (.*)"))
async def download_video(Lethal):
    x = await eor(Lethal, "ѕєαя¢нιиg...")
    url = Lethal.pattern_match.group(1)
    if not url:
    ...
    try:
        ...
        await Lethal(JoinChannelRequest(sung))
    except BaseException:
        pass
    upteload = """
Uploading...
...
@Lethal.on(admin_cmd(pattern="vsong (.*)"))
async def download_video(Lethal):
    x = await eor(Lethal, "Processing..")
    url = Lethal.pattern_match.group(1)
    if not url:
