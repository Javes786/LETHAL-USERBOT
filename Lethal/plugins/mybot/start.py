...
from Lethal import CUSTOM_PMPERMIT
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** Lethal Logs",
        buttons=[
            [Button.url("View Online", f"{url}")],
            [Button.url("Crashed?", "t.me/DestroyXSupport")]
        ])
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Lethal"))
          )  # pylint: disable=oof
async def settings(event):
    await event.delete()
    await tgbot.send_message(event.chat_id,
