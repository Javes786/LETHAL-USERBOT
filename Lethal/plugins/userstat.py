...
@Lethal.on(admin_cmd(pattern=f"ustat(?: |$)(.*)"))
@Lethal.on(sudo_cmd(pattern=f"ustat(?: |$)(.*)", allow_sudo=True))
async def _(event):
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        lethal = await event.reply("`Processing...`")
    else:
        lethal = await event.edit("`Processing...`")
    if event.fwd_from:
        return
    tuser, rdhs = await get_full_user(event)
    if tuser is None:
        await lethal.edit("Error! Please mention user to check his stats!!")
        return False
