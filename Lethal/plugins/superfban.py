...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await Lethal.download_media(
    ...
    if not len(fedList):
        for a in range(3):
            async with Lethal.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await Lethal.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.EXCLUDE_FED and fed in excludeFed:
            await Lethal.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await Lethal.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await Lethal.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
...
@Lethal.on(admin_cmd("superunfban ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    for a in range(3):
        async with Lethal.conversation("@MissRose_bot") as bot_conv:
            ...
            if "make a file" in response.text:
                ...
                fedfile = await bot_conv.get_response()
                if fedfile.media:
                    downloaded_file_name = await Lethal.download_media(fedfile, "fedlist")
                    file = open(downloaded_file_name, "r")
                    lines = file.readlines()
                    for line in lines:
    ...
    for fed in fedList:
        await Lethal.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(5)
        await Lethal.send_message(chat, f"/unfban {FBAN}")
        await asyncio.sleep(5)
